...
def load_data():
    file_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'BASE PARA DASH TESTE.xlsx')

    # STATUS DOBRADEIRA
    status_dobradeira = pd.read_excel(file_path, sheet_name=0, skiprows=7, nrows=11, usecols="A:C")
    status_dobradeira.columns = status_dobradeira.iloc[0]
    status_dobradeira = status_dobradeira[1:].reset_index(drop=True)

    # PEÇAS FALTANTES
    pecas_faltantes_solda = pd.read_excel(file_path, sheet_name=0, skiprows=20, nrows=11, usecols="A:E")
    ...
    estoque_intermediario = pd.read_excel(file_path, sheet_name=0, skiprows=32, nrows=11, usecols="A:E")
